...
import {
  CheckCircle, FolderOpen, Calendar,
...
const Dashboard: React.FC = () => {
  ...
  const activityChartData = React.useMemo(() => {
    if (taskData?.labels && Array.isArray(taskData.labels)) {
      return taskData.labels.map((label: string, idx: number) => ({
        date: label,
  ...
  const codingChartData = React.useMemo(() => {
    if (codingData?.labels && Array.isArray(codingData.labels)) {
      return codingData.labels.map((label: string, idx: number) => ({
        date: label,
  ...
  const safeStats = {
    ...
    total_coding_hours: stats?.total_coding_hours || 0,
  };
  ...
  const statsCards = [
    ...
    {
      title: 'Productivity',
      value: `${safeStats.productivity_score}%`,
      icon: Zap,
    ...
    },
    {
      ...
      value: safeStats.completed_projects,
      icon: Award,
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-blue-100">
              You've completed {safeStats.completed_tasks} out of {safeStats.total_tasks} tasks
            </p>
            <div className="mt-3 flex items-center space-x-2">
              <Award className="h-5 w-5 text-yellow-300" />
              <span className="text-sm font-medium">
                {safeStats.productivity_score > 70 ? 'Great progress!' : 'Keep pushing! 🔥'}
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {activityChartData.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-[300px] text-gray-500">
              <div className="text-center">
                ...
                <p className="text-sm">Create tasks to see your activity</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 md:p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          {codingChartData.length > 0 && codingChartData.some((item: { hours: number }) => item.hours > 0) ? (
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={codingChartData}>
                ...
                <Tooltip
                  formatter={(value) => {
                    ...
                    return [`${safeValue} hours`, "Coding Time"];
                  }}
                />
                <Line 
                  type="monotone" 
